...
void akimaFixedGrid_precompute_float
(
    /* INPUTS:  */
...
{
    akimaCoefficients_float(gridVectors, gridValues, gridSize,
...
void akimaFixedGrid_interpolate_float
(
    /* INPUTS:  */
    const MFL_INTERP_UINT         N,
    const MFL_INTERP_UINT*        gridSize,
...
{
    akimaEvaluationViaHermiteBasis_float(
                                            gridVectors,
                                            gridSize,
                                            N,
                                            extrapMethod,
...
void akimaFixedQuery_precompute_float
(
    /* INPUTS:  */
    const MFL_INTERP_UINT         N,
    const MFL_INTERP_UINT*        gridSize,
    ...
    for (kk = 0; kk < numQ; ++kk) {
        ...
        for (jj = 0; jj < N; ++jj) {
            ...
            b = binsXq ? binsXq[jj][kk]
                       : akimaFindGridInterval1D_float(gridVectors[jj],
                                                                       gridSize[jj],
                                                                       Xq[jj][kk]);

            /* Compute the first and second Hermite basis coefficients: */
            jj2 = jj*2*numQ;
            akimaHermiteBasis1D_float(
                                        gridVectors[jj],
                                        gridSize[jj],
                                        extrapMethod,
                                        noDerivatives,
...
void akimaFixedQuery_interpolate_float
(
    /* INPUTS:  */
    const MFL_INTERP_UINT         N,
    const MFL_INTERP_UINT*        gridSize,
...
)
{
    ...
    akimaCoefficients_float(gridVectors, gridValues, gridSize, N, tmp, work2, work1);
...
)
{
    ...
    for (i = 0; i < N; ++i) {
        for (k = 0; k < numQ; ++k) {
            binsXq[i][k] = akimaFindGridInterval1D_float(gridVectors[i],gridSize[i],Xq[i][k]);
        }
    }
}
...
void akimaFixedGrid_precompute_1D_float
(
    /* INPUTS:  */
...
void akimaFixedGrid_interpolate_1D_float
(
    /* INPUTS:  */
    const MFL_INTERP_UINT        nx,
    const float* x,
...
{
    akimaEvaluationViaHermiteBasis1D_float(
                                            x,
                                            nx,
                                            extrapMethod,
                                            coefficients,
...
void akimaFixedQuery_precompute_1D_float
(
    /* INPUTS:  */
    const MFL_INTERP_UINT        nx,
    const float* x,
...
)
{
    ...
    for (kk = 0; kk < numQ; ++kk) {
        ...
        b = binsXq ? binsXq[kk] : akimaFindGridInterval1D_float(x,nx,xq[kk]);
...
void akimaFixedQuery_interpolate_1D_float
(
    /* INPUTS:  */
    const MFL_INTERP_UINT        nx,
    const float* x,
...
)
{
    MFL_INTERP_UINT k;
    for (k = 0; k < numQ; ++k) {
        binsXq[k] = akimaFindGridInterval1D_float(x,nx,xq[k]);
    }
}
