...
)
{
    ...
    memset(finiteDiffs,0,numelFiniteDiffs*sizeof(float));
    
    /* The first finite difference is a copy of the gridValues */
    memcpy(finiteDiffs,gridValues,gridNumel*sizeof(float));
        ...
        for (jjF = iiF, jjFD = iiFD; jjF < iiF + strideF; ++jjF, ++jjFD) {
            ...
            for (kk = 1; kk < numelVectorF; ++kk) {
                ...
                if (x) { /* Used when computing 1-D Akima finite differences FD */
                     ...
                     */
                    FD[skkFD] = akimaAbs_float( f2 - f1 ) +
                                akimaAbs_float( f2/2 + f1/2 );
                }
            }
            ...
            if (x) { /* Used when computing 1-D Akima finite differences FD */
                ...
                FD[jjFDend] = akimaAbs_float( f2 - f1 ) +
                              akimaAbs_float( f2/2 + f1/2 );
            }
        }
    }
