...
)
{
    ...
    float* workspaceDerivatives;
    float* workspaceFiniteDiffs;
    float* workspaceWeights;
    ...
    memcpy(coefficients,gridValues,gridNumel*sizeof(float));
...
{
    MFL_INTERP_UINT ii;
    float sL, sU, wL, wU, denW, s1, s2;
    float* s; /* slopes */
    float* w; /* weights */
    float* d; /* modified Akima derivative estimates */


    /* The first Akima coefficient is simply a copy of the grid value v */
    memcpy(coefficients,v,nx*sizeof(float));
     ...
     */
    for (ii = 1; ii < nx+1; ++ii) {
        w[ii] = akimaAbs_float( s[ii] - s[ii-1] ) + 
                akimaAbs_float( s[ii]/2 + s[ii-1]/2 );
    }
    
    /* Extrapolate the first weight */
    s2 = s[0];
    s1 = 2*s[0] - s[1];
    w[0] = akimaAbs_float( s2 - s1 ) +
           akimaAbs_float( s2/2 + s1/2 );

    /* Extrapolate the last weight */
    s2 = s[nx];
    s1 = 2*s[nx] - s[nx-1];
    w[nx+1] = akimaAbs_float( s2 - s1 ) +
              akimaAbs_float( s2/2 + s1/2 );
