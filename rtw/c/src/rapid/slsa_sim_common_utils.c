...
//extern int_T gblExecutionInfo.gblObjects_.inportComplex[];
/* inport interpolation flag */
//extern int_T gblInportInterpoFlag[];
/* simulink data type idx of input port signal */
//extern int_T gblExecutionInfo.gblObjects_.inportDataTypeIdx[];
...
{     
    ...
    if (numDimsValues == 2){
        if (dimsValues[1] != (size_t)gblExecutionInfo.gblObjects_.inportDims[inportIdx*2]){
    ...
    if (numDimsValues == 3){
      if (dimsValues[0] != (size_t)gblExecutionInfo.gblObjects_.inportDims[inportIdx*2] 
            ||dimsValues[1] != (size_t)gblExecutionInfo.gblObjects_.inportDims[inportIdx*2+1]){
            (void)sprintf(errmsg,"Dimension of signal #%d from MAT-file not " 
                          "equal the dimension of the corresponding inports "
    ...
    {
        int_T            dTypeID = gblExecutionInfo.gblObjects_.inportDataTypeIdx[inportIdx]; 
    ...
    /* 5. Check for port complexity mismatch */
    {
        ...
        boolean_T isComplexFromFile  = (bool)gblExecutionInfo.gblObjects_.inportComplex[inportIdx];
...
{                                          
    ...
    if (inputTimePtr != NULL) {
        ...
        if (numOfTimePoints == 0) {
            if(inportIdx < 0) {
                for (i = 0; i < gblExecutionInfo.gblObjects_.numRootInportBlks; ++i) {
                    if (gblExecutionInfo.gblObjects_.inportInterpoFlag[i] != 0) {
                        (void)sprintf(errmsg, 
                    ...
                    if (gblExecutionInfo.gblObjects_.inportContinuous[i] == 1) {
                        (void)sprintf(errmsg, 
                ...
                if (gblExecutionInfo.gblObjects_.inportInterpoFlag[inportIdx] != 0){
                    (void)sprintf(errmsg, 
                ...
                if (gblExecutionInfo.gblObjects_.inportContinuous[inportIdx] == 1) {
                    (void)sprintf(errmsg, 
...
{   
    ...
    if (gblExecutionInfo.gblObjects_.numRootInportBlks == 0) {
        return NULL;
    } else {
    ...
    if (isRaccel) {        
        ...
        if (nrows != 1 || ncols != gblExecutionInfo.gblObjects_.numRootInportBlks) {
        ...
        (void)memcpy(periodicFunctionCallInports, mxGetLogicals(mxInputPtr),
                     sizeof(mxLogical)*gblExecutionInfo.gblObjects_.numRootInportBlks);
    ...
    if (numVarInMatFile == 1 && gblExecutionInfo.gblObjects_.numRootInportBlks > 0 && 
        (gblExecutionInfo.gblObjects_.inportDataTypeIdx[0] != SS_FCN_CALL)) {
        ...
        if (!isStruct) {
            ...
            if (ncols != gblExecutionInfo.gblObjects_.numModelInputs + 1) {
                (void)sprintf(errmsg,
                              "The number of columns of matrix variable from "
                              "MAT-file '%s' must equal %d, which is the "
                              "number of root inports plus 1.\n ", 
                              inportFileName, gblExecutionInfo.gblObjects_.numModelInputs + 1);
                goto EXIT_POINT;
            }
        
            for (i = 0; i < gblExecutionInfo.gblObjects_.numRootInportBlks; ++i) {
                /* check complexity, only real */         
                if (gblExecutionInfo.gblObjects_.inportComplex[i] == 1) {
                ...
                if (gblExecutionInfo.gblObjects_.inportDataTypeIdx[i] != SS_DOUBLE) {  
                    (void)sprintf(errmsg,"Data type of matrix variable from "
                ...
                if(gblExecutionInfo.gblObjects_.inportDims[2*i + 1] != 1){
                    (void)sprintf(errmsg, 
            ...
            *matFileDataFormat = SINGLEVAR_MATRIX;
            ...
            if(numInportSignals != gblExecutionInfo.gblObjects_.numRootInportBlks){
                (void)sprintf(errmsg, 
                              "MAT-file '%s' contains %d signals while model "
                              "contains %d inports, they must be equal.\n ",
                              inportFileName, numInportSignals, 
                              gblExecutionInfo.gblObjects_.numRootInportBlks);
                goto EXIT_POINT;
            }
        }
        *matFileDataFormat = SINGLEVAR_STRUCT;
         ...
         */
        for (inportIdx = 0; inportIdx < gblExecutionInfo.gblObjects_.numRootInportBlks; ++inportIdx){
    ...
    } else if (numVarInMatFile != gblExecutionInfo.gblObjects_.numRootInportBlks) {
        (void)sprintf(errmsg,"MAT-file '%s' contains %d signals "
                      "while model contain %d inports. ",
                      inportFileName, numVarInMatFile, gblExecutionInfo.gblObjects_.numRootInportBlks);
        goto EXIT_POINT;               
    } else {
        *matFileDataFormat = MULTIPLEVAR_LIST;
        ...
        for (inportIdx = 0; inportIdx < gblExecutionInfo.gblObjects_.numRootInportBlks; ++inportIdx){
            ...
            if (!mxIsStruct(mxInputPtr)) {
                int_T portWidth  = 
                    gblExecutionInfo.gblObjects_.inportDims[inportIdx*2]*
                    gblExecutionInfo.gblObjects_.inportDims[inportIdx*2 + 1];
                nrows = (int) mxGetM(mxInputPtr);
                ncols = (int) mxGetN(mxInputPtr);    
                mxIDfromFile  = mxGetClassID(mxInputPtr);
                ...
                if ((gblExecutionInfo.gblObjects_.inportDataTypeIdx[inportIdx] != SS_FCN_CALL &&
                     ncols != portWidth + 1) ||
                    (gblExecutionInfo.gblObjects_.inportDataTypeIdx[inportIdx] == SS_FCN_CALL &&
                     ncols != portWidth)) {
                    (void)sprintf(errmsg,
                ...
                if (gblExecutionInfo.gblObjects_.inportComplex[inportIdx] == 1) {
                    printf("*** Warning: Signal type of matrix variable from "
                           "MAT-file %s can only be real while signal type "
                           "of inport %d is set to complex. The imaginary part " 
                ...
                if (gblExecutionInfo.gblObjects_.inportDataTypeIdx[inportIdx] != SS_DOUBLE &&
                    gblExecutionInfo.gblObjects_.inportDataTypeIdx[inportIdx] != SS_FCN_CALL) {  
                    (void)sprintf(errmsg,"Data type of matrix variable from "
                                  "MAT-file %s can only be double while data "
                                  "type of inport %d is not set to double.\n", 
...
static const char* setGblInportTUtableElement(int_T inportIdx,
    ...
    int_T portWidth  = 
        gblExecutionInfo.gblObjects_.inportDims[inportIdx*2]*
    gblExecutionInfo.gblObjects_.inportDims[inportIdx*2 + 1];
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ur = NULL;
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ui = NULL;
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].time = NULL; 
    
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].rtComplex = isComplex ? 1 : 0;    
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].isPeriodicFcnCall = isPeriodicFcnCall;
        
    /* set TU Table attribute */
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].nTimePoints = (int_T) numOfTimePoints;        
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].uDataType = 
        gblExecutionInfo.gblObjects_.inportDataTypeIdx[inportIdx]; 
    ...
    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].currTimeIdx = (int*)calloc(1, sizeof(int));
    if (gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].currTimeIdx == NULL) {
            return "Memory allocation error";
    }

    *(gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].currTimeIdx) =
        (inportTimeDataPtr && !isPeriodicFcnCall) ? 0 : -1;  

    if (numOfTimePoints == 0) return NULL;

    gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].time = inportTimeDataPtr;

    if (elementSize > 0) {
        /* allocate memory */
        gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ur = 
            (char*)calloc(numOfTimePoints*portWidth,elementSize);     
        
        if (gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ur == NULL) {
            return "Memory allocation error";
        }            
        
        if(gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].rtComplex == 1){
            gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ui = 
                (char*)calloc(numOfTimePoints*portWidth,elementSize);     
            if (gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ui == NULL) {
                return "Memory allocation error"; 
            }
        }          
        ...
        (void)memcpy(gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ur, matDataRe, 
                     elementSize*numOfTimePoints*portWidth);
    
        if (isComplex){                    
            (void)memcpy(gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ui, matDataIm, 
                         elementSize*numOfTimePoints*portWidth);
        }
    } else {
        gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ur = matDataRe;
        gblExecutionInfo.gblObjects_.inportTUtables[inportIdx].ui = matDataIm;
    }
    return NULL;
}
                                              ...
                                              )
{
    ...
    gblExecutionInfo.gblObjects_.inportTUtables = (rtInportTUtable*)
        malloc(sizeof(rtInportTUtable)*gblExecutionInfo.gblObjects_.numRootInportBlks);
    
    if (gblExecutionInfo.gblObjects_.inportTUtables == NULL) {
    ...
    if (matFileDataFormat == SINGLEVAR_MATRIX) {
        ...
        for (inportIdx = 0; inportIdx < gblExecutionInfo.gblObjects_.numRootInportBlks; ++inportIdx) {
            ...
            portWidth  = 
                gblExecutionInfo.gblObjects_.inportDims[inportIdx*2]*gblExecutionInfo.gblObjects_.inportDims[inportIdx*2 + 1];
        ...
        for (inportIdx = 0; inportIdx < gblExecutionInfo.gblObjects_.numRootInportBlks; ++inportIdx) {
            ...
            isComplex = ((gblExecutionInfo.gblObjects_.inportComplex[inportIdx] != 
                          mxIsComplex(mxInportSignalValues))? 
                         false : (bool) gblExecutionInfo.gblObjects_.inportComplex[inportIdx]);  
                                   ...
                                   FrFInfo * frFInfo)
{
     ...
     ******************************************************************/
    {
        ...
        for (i=0; i<gblExecutionInfo.gblObjects_.numFrFiles; i++) {
            if (gblExecutionInfo.gblObjects_.frFNamepair[i].newName != NULL && \
                strcmp(origFileName, gblExecutionInfo.gblObjects_.frFNamepair[i].oldName)==0) {
                frFInfo->newFileName = gblExecutionInfo.gblObjects_.frFNamepair[i].newName; /* remap */
                gblExecutionInfo.gblObjects_.frFNamepair[i].remapped = 1;
                break;
            }
        }
    }
...
{
    FreeFNamePairList(gblExecutionInfo.gblObjects_.toFNamepair, gblExecutionInfo.gblObjects_.numToFiles);
    FreeFNamePairList(gblExecutionInfo.gblObjects_.frFNamepair, gblExecutionInfo.gblObjects_.numFrFiles);
    ...
    if(gblExecutionInfo.gblObjects_.numRootInportBlks>0){
        ...
        if (gblExecutionInfo.gblObjects_.inportTUtables!= NULL){
            for(i=0; i< gblExecutionInfo.gblObjects_.numRootInportBlks; i++){
                ...
                if(gblExecutionInfo.gblObjects_.inportTUtables[i].time != NULL){
                    if(matFileFormat == 2){
                        free(gblExecutionInfo.gblObjects_.inportTUtables[i].time);
                    }else{
                        if(i==0){
                            free(gblExecutionInfo.gblObjects_.inportTUtables[0].time);
                        }
                    }                    
                    gblExecutionInfo.gblObjects_.inportTUtables[i].time = NULL;
                }
                
                if(gblExecutionInfo.gblObjects_.inportTUtables[i].ur != (char*)NULL){
                    free(gblExecutionInfo.gblObjects_.inportTUtables[i].ur);
                    gblExecutionInfo.gblObjects_.inportTUtables[i].ur = NULL;
                }
                if(gblExecutionInfo.gblObjects_.inportTUtables[i].ui != (char*)NULL){
                    free(gblExecutionInfo.gblObjects_.inportTUtables[i].ui);
                    gblExecutionInfo.gblObjects_.inportTUtables[i].ui = NULL;
                }          
                if(gblExecutionInfo.gblObjects_.inportTUtables[i].currTimeIdx != (int*)NULL){
                    free(gblExecutionInfo.gblObjects_.inportTUtables[i].currTimeIdx);
                    gblExecutionInfo.gblObjects_.inportTUtables[i].currTimeIdx = NULL;
                }
            }
            free(gblExecutionInfo.gblObjects_.inportTUtables);
        }    
    }
...
const char *rt_RapidCheckRemappings(void)
{
    ...
    for (i = 0; i < gblExecutionInfo.gblObjects_.numFrFiles; i++) {
        if (gblExecutionInfo.gblObjects_.frFNamepair[i].oldName != NULL && gblExecutionInfo.gblObjects_.frFNamepair[i].remapped==0){
            return("one or more -f switches from file names do not exist in "
                   "the model");
        }
    }

    for (i = 0; i < gblExecutionInfo.gblObjects_.numToFiles; i++) {
        if (gblExecutionInfo.gblObjects_.toFNamepair[i].oldName != NULL && gblExecutionInfo.gblObjects_.toFNamepair[i].remapped==0){
...
{
    return gblExecutionInfo.gblObjects_.matSigstreamLoggingFilename;
}
...
{
    return gblExecutionInfo.gblObjects_.matSigLogSelectorFilename;
}
