...
/*
  ...
  simulation.
 */
PMF_DEPLOY_STATIC McLinearAlgebraData* rtw_linalg_create_data(
    PmAllocator*             allocatorPtr,
    const PmSparsityPattern* jacobian_pattern_ptr) {

    McLinearAlgebraData* ne_la_data =
        (McLinearAlgebraData*)pm_allocator_alloc(allocatorPtr, sizeof(McLinearAlgebraData), 1);
    ...
    ne_la_data->mLU =
        (real_T*)pm_allocator_alloc(ne_la_data->mAllocatorPtr, sizeof(real_T),
                                    jacobian_pattern_ptr->mNumRow * jacobian_pattern_ptr->mNumCol);

    ne_la_data->mLinvB = (real_T*)pm_allocator_alloc(ne_la_data->mAllocatorPtr, sizeof(real_T),
                                                     jacobian_pattern_ptr->mNumRow);

    ne_la_data->mPivotIndices = (int32_T*)pm_allocator_alloc(
        ne_la_data->mAllocatorPtr, sizeof(int32_T),
        ((((int32_T)jacobian_pattern_ptr->mNumRow) < ne_la_data->mNumCol) ? ne_la_data->mNumRow
                                                                          : ne_la_data->mNumCol));
    return ne_la_data;
}

/* Perform the LU factorization */
PMF_DEPLOY_STATIC McLinearAlgebraStatus rtw_linalg_numeric(McLinearAlgebra* ne_la,
                                                           const real_T*    Ax) {
    McLinearAlgebraData* ne_la_data = ne_la->mPrivateData;

    populate_full_matrix_from_sparsitypattern(ne_la_data->mLU, ne_la_data->mSparsityPatternPtr, Ax,
    ...
    {
        int32_T diagLen  = ((ne_la_data->mNumRow < ne_la_data->mNumCol) ? ne_la_data->mNumRow
                                                                        : ne_la_data->mNumCol);
...
PMF_DEPLOY_STATIC McLinearAlgebraStatus rtw_linalg_solve(McLinearAlgebra* ne_la,
    ...
    rt_ForwardSubstitutionRR_Dbl(ne_la_data->mLU, B, ne_la_data->mLinvB, ne_la_data->mNumRow, 1,
                                 ne_la_data->mPivotIndices, true);
    rt_BackwardSubstitutionRR_Dbl(
        ne_la_data->mLU + ((ne_la_data->mNumRow * ne_la_data->mNumRow) - 1),
        ne_la_data->mLinvB + (ne_la_data->mNumRow - 1), dy, ne_la_data->mNumRow, 1, false);
    UNUSED_PARAMETER(Ax);
    return MC_LA_OK;
}
...
size_t rtw_linalg_memusage(const McLinearAlgebra* ne_la) {
    ...
    return
        ...
        sizeof(*ne_la) + sizeof(*ne_la_data) + numrow * numcol * sizeof(real_T) +
        numrow * sizeof(real_T) + ((numrow < numcol) ? numrow : numcol) * sizeof(int32_T);
}
...
create_rtw_linear_algebra_complete(const McLinearAlgebraFactory* factory,
    ...
    McLinearAlgebra* la = (McLinearAlgebra*)pm_allocator_alloc(alloc, sizeof(McLinearAlgebra), 1);
