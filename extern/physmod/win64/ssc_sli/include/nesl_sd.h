...
#define LOCAL_SIM_DATA_ASSIGN(_name, _type) sim_data->m##_name = local_sim_data_##_name;

PMF_DEPLOY_STATIC NeslSimulationData* nesl_create_simulation_data(void) {
    PmAllocator*        a = pm_default_allocator();
    NeslSimulationData* sim_data =
        (NeslSimulationData*)pm_allocator_alloc(a, sizeof(NeslSimulationData), 1);
    NeslSimulationDataData* data =
        (NeslSimulationDataData*)pm_allocator_alloc(a, sizeof(NeslSimulationDataData), 1);
