...
#define __slmsg_POOLED_FREE(pool, ptr) \
    ((void)(__slmsg_Mem_free((ptr), __FILE__, __LINE__), (ptr) = 0))
#endif

#define SLMSG_CIRCULAR_INDEX(index, capacity) (((capacity) == 0) ? (index) : ((index) % (capacity)))
...
void* __slmsg_Mem_calloc(slMsgPoolSize count,
                         slMsgPoolSize nbytes,
                         const char_T* file,
                         int_T a_line);
...
void __slmsg_MemPool_Add(__slmsg_MemPool* pool,
    ...
    for (idx = 0; idx < numUnits; ++idx) {
        __slmsg_PoolListNode* pCurUnit =
            (__slmsg_PoolListNode*)((char*)chunk->fMemBlock + idx * unitNodeSize);
...
void* __slmsg_MemPool_Alloc(__slmsg_MemPool* pool, ulong_T nbytes) {
    ...
    if (NULL == pool->fFreeListHead) {
        ...
        if (pool->fCanMalloc) { /* Grow the memory pool */
            ...
            const slMsgPoolSize unitNodeSize =
                (slMsgPoolSize)pool->fUnitSize + sizeof(__slmsg_PoolListNode);
...
int8_T __slmsg_MemPoolForData(__slmsg_MemPool* pool, void* ptr) {
    ...
    do {
        memFromPool = ((chunk->fMemBlock < ptr) &&
...
void __slmsg_private_MemPool_Free(__slmsg_MemPool* pool, void** ptrIn) {
    ...
    if (memFromPool) {
        ...
        pCurUnit = (__slmsg_PoolListNode*)((char*)ptr - sizeof(__slmsg_PoolListNode));
...
void __slmsg_private_MemPool_Destroy(__slmsg_MemPool* pool) {
    ...
    printf("-- Number of blocks cleaned   = %d\n", (*pool)->fStatNumAlloc - (*pool)->fStatNumFree);
#endif
...
void _slMsgSvcMarkQueueDropMsg(slMsgManager* mgr, slMsgQueueId qid, slMessage* msg);
...
slMessage* _slMsgSvcPeekMsgFromQueueAtIndex(slMsgManager* msgMgr,
                                            slMsgQueueId queueId,
                                            int msgIndex);
...
void _slMsgSvcInitPool(slMsgMemPool* memPool,
    ...
    if (mallocAllowed) {
        ...
        memPool->fMemChunk = (__slmsg_MemChunk*)__slmsg_SYSTEM_ALLOC(sizeof(__slmsg_MemChunk));
    } else {
#endif
    ...
    __slmsg_MemPool_Add(memPool, numUnits, (ulong_T)dataSize, memBuffer);
}
...
slMessage* _slMsgAddToQueue(slMsgManager* msgMgr, slMessage* msg, slMsgQueueId queueId) {
    ...
    if (q->fTail == NULL) {
        ...
        case SLMSG_SYSPRIORITY_QUEUE_DESCENDING: {
            ...
            while (thisMsg != NULL) {
                const real_T thisVal = _slMsgGetMsgPriorityValWithCast(thisMsg, q);
...
/* Mark queue drop a message for diagnostics and logging*/
void _slMsgSvcMarkQueueDropMsg(slMsgManager* msgMgr, slMsgQueueId qid, slMessage* msg) {
    slMsgQueue* q = NULL;

    __slmsg_assert(msgMgr != NULL);
    __slmsg_assert(msgMgr->fQueues != NULL);
    __slmsg_assert(qid != SLMSG_UNSPECIFIED);

    q = &(msgMgr->fQueues[qid]);
    ++q->_fNumDropped;
    if (msg && q->_fInstrumentDropObj != NULL) {
        _slmsg_instrument_drop(msg, qid, q->_fInstrumentDropObj);
    }
}
...
slMessage* _slMsgSvcSRSWSendMsg(slMsgManager* msgMgr, slMessage* msg, slMsgQueueId queueId) {
    ...
    if (SLMSG_CIRCULAR_INDEX(q->fSRSWFIFOQueue.fCircularTail + 1,
                             q->fSRSWFIFOQueue.fCircularCapacity) !=
        q->fSRSWFIFOQueue.fCircularHead) {

        /* copy msg payload */
        void* dst = (q->fSRSWFIFOQueue.fCircularArray +
                     (q->fSRSWFIFOQueue.fCircularTail * q->fSRSWFIFOQueue.fCircularChunkSize));
        ...
        q->fSRSWFIFOQueue.fCircularTail = SLMSG_CIRCULAR_INDEX(q->fSRSWFIFOQueue.fCircularTail + 1,
                                                               q->fSRSWFIFOQueue.fCircularCapacity);
...
boolean_T _slMsgSvcIsMsgInQueue(slMsgManager* msgMgr, slMsgQueueId queueId, slMessage* inMsg) {
    slMessage* msg = NULL;
    slMsgQueue* q;
    int msgCounter;
...
slMessage* _slMsgSvcSRSWReadMsgFromQueue(slMsgManager* msgMgr,
                                         slMsgQueueId queueId,
                                         boolean_T pop) {
    slMessage* msg = NULL;
#ifdef SLMSG_INCLUDE_TASK_TRANSITION_QUEUE
    ...
    if (q->fSRSWFIFOQueue.fCircularHead != q->fSRSWFIFOQueue.fCircularTail) {
        msg = _slMsgSvcCreateMsg(
            msgMgr,
            (q->fSRSWFIFOQueue.fCircularArray +
             (q->fSRSWFIFOQueue.fCircularHead * q->fSRSWFIFOQueue.fCircularChunkSize)),
            q->fDataSize, queueId);
        ...
        if (pop) {
            q->fSRSWFIFOQueue.fCircularHead = SLMSG_CIRCULAR_INDEX(
                q->fSRSWFIFOQueue.fCircularHead + 1, q->fSRSWFIFOQueue.fCircularCapacity);
            q->fIsOverflow = 0;
        }
    }
#endif
...
void slMsgSvcCreateFIFOMsgQueue(void* msgMgr,
                                ...
                                slMsgMemPoolId payloadMemPoolId) {
    slMsgManager* msgMgrT = (slMsgManager*)msgMgr;
    _slMsgCreateMsgQueue(msgMgrT, id, SLMSG_FIFO_QUEUE, capacity, (slMsgQueueDropPolicy)dropPolicy,
                         dataSize, SLMSG_UNSPECIFIED, 0, messageMemPoolId, payloadMemPoolId);
}
...
void slMsgSvcCreateSRSWFIFOMsgQueue(void* msgMgr,
    ...
    _slMsgCreateSRSWMsgQueue(msgMgrT, (uint8_T*)sharedArray, id, SLMSG_SRSW_LOCK_FREE_FIFO_QUEUE,
                             capacity, dropPolicy, dataSize, SLMSG_UNSPECIFIED, 0,
                             readerMessageMemPoolId, writerMessageMemPoolId, readerPayloadMemPoolId,
...
void slMsgSvcCreateLIFOMsgQueue(void* msgMgr,
                                ...
                                slMsgMemPoolId payloadMemPoolId) {
    slMsgManager* msgMgrT = (slMsgManager*)msgMgr;
    _slMsgCreateMsgQueue(msgMgrT, id, SLMSG_LIFO_QUEUE, capacity, (slMsgQueueDropPolicy)dropPolicy,
                         dataSize, SLMSG_UNSPECIFIED, 0, messageMemPoolId, payloadMemPoolId);
}
...
void slMsgSvcCreatePriorityMsgQueue(void* msgMgr,
    ...
    _slMsgCreateMsgQueue(msgMgrT, id, qType, capacity, (slMsgQueueDropPolicy)dropPolicy, dataSize,
                         priorityDataType, priorityDataOffset, messageMemPoolId, payloadMemPoolId);
}
...
/* Create a new message */
void* slMsgSvcCreateMsg(void* msgMgr,
                        const void* data,
                        slMsgDataSize dataSize,
                        slMsgQueueId queueId) {
    /* unconnected sender */
...
slMessage* _des_slMsgSvcCloneMsg(slMsgManager* mgr, slMessage* msg) {
    slMessage* new_msg =
        _slMsgSvcCreateMsgWithId(mgr, msg->fData, msg->fDataSize, msg->fQueueId, msg->fId);
...
real_T _des_slMsgSvcGetPriorityValWithCast(slMsgManager* mgr,
                                           slMessage* msg,
                                           slMsgQueueId queueId) {
    real_T val = 0;
    slMsgQueue* q = NULL;
    q = &(mgr->fQueues[queueId]);
    if (q->fType == SLMSG_PRIORITY_QUEUE_ASCENDING || q->fType == SLMSG_PRIORITY_QUEUE_DESCENDING ||
