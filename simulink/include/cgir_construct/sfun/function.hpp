...
inline void Function::initialize(const char* name,
    ...
    fpImpl = SFun_Function_ctor_c_c_T_T_sz(name, headerFile, outputType.getImpl(),
                                           &inputTypeImpls[0], numInputTypes);
    verify(fpImpl);
}
...
inline Value Function::operator()(const Value& arg0, const Value& arg1) const {
    const SFun_Value_Impl* args[] = {arg0.getImpl(), arg1.getImpl()};
    return doCall(args, sizeof(args) / sizeof(args[0]));
}

inline Value Function::operator()(const Value& arg0, const Value& arg1, const Value& arg2) const {
    const SFun_Value_Impl* args[] = {arg0.getImpl(), arg1.getImpl(), arg2.getImpl()};
    return doCall(args, sizeof(args) / sizeof(args[0]));
}

inline Value Function::operator()(const Value args[], std::size_t numArgs) const {
    std::vector<const SFun_Value_Impl*> argImpls(numArgs);
    for (std::size_t i = 0; i != numArgs; ++i) {
        argImpls[i] = args[i].getImpl();
    }

    return doCall(numArgs == 0 ? NULL : &argImpls[0], numArgs);
}

inline Value Function::operator()(const std::list<Value>& args) const {
    std::vector<const SFun_Value_Impl*> argImpls;
    for (std::list<Value>::const_iterator pArg = args.begin(); pArg != args.end(); ++pArg) {
        argImpls.push_back(pArg->getImpl());
    }

    return doCall(argImpls.empty() ? NULL : &argImpls[0], argImpls.size());
}

inline Value Function::doCall(const SFun_Value_Impl* args[], std::size_t numArgs) const {
    SFun_Value_Impl* pResult = SFun_Function_op_fnCall_V_sz(fpImpl, args, numArgs);
